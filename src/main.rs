...
enum Token {
	...
	Decrease, DecreaseX,
	Differences, DifferencesX, // 4,2,1 -> 2,1
	Digits, DigitsX,
	DivideInt, DivideIntX,
	// TODO: rename to copy?
	DuplicateFromIndex, // DuplicateFromIndexX,
	DuplicateStack, // 1 2 -> 1 2 1 2 // DuplicateStackX, // noop
	DuplicateToIndex, // DuplicateToIndexX,
	DuplicateToBottom, // DuplicateToBottomX,
	DuplicateTop, // DuplicateTopX,
	DuplicateUnder, // DuplicateUnderX,
	First, FirstX,
	Head, HeadX, // everything but last
	Increase, IncreaseX,
	IndexOfMaxFirst, IndexOfMaxFirstX,
	IndexOfMaxLast, IndexOfMaxLastX,
	IndexOfMinFirst, IndexOfMinFirstX,
	IndexOfMinLast, IndexOfMinLastX,
	// InsertValueAtIndex, // 0,1,2,3  5 2 insertat -> 0,1,5,2,3
	// InsertAtIndexValue, // 0,1,2,3  2 5 insertat -> 0,1,5,2,3
	IsAllEqual, IsAllEqualX, // 2,2,2 -> 1
	// IsAllNotEqual, // IsAllNotEqualX, // ?
	IsEqual, IsEqualX, // 42 42 -> 1
	IsNotEqual, IsNotEqualX, // 42 137 -> 1
	...
	Max, MaxX,
	Min, MinX,
	ModuloFake, ModuloFakeX,
	ModuloRemEuclid, ModuloRemEuclidX,
	...
	Product, ProductX, // 1,2,3 -> 6
	Products, ProductsX, // 1,2,3 -> 1,2,6
	// TODO: range: to/from? (aka ascending/descending)
	Range0Excluding, Range0ExcludingX,
	Range0Including, Range0IncludingX,
	Range1Excluding, Range1ExcludingX,
	Range1Including, Range1IncludingX,
	Reverse, ReverseX,
	ReverseStack, ReverseStackX, // 1 2 3 -> 3 2 1
	// TODO: SliceArr 0,1,2,3,4,5,6 2,5 slicearr -> 2,3,4,5
	SliceExcludingExcluding, SliceExcludingExcludingX,
	SliceExcludingIncluding, SliceExcludingIncludingX,
	SliceExcludingFrom, SliceExcludingFromX,
	SliceExcludingTo, SliceExcludingToX,
	SliceIncludingExcluding, SliceIncludingExcludingX,
	SliceIncludingIncluding, SliceIncludingIncludingX,
	SliceIncludingFrom, SliceIncludingFromX,
	SliceIncludingTo, SliceIncludingToX,
	Sort, SortX,
	Split, SplitX, // 1,2,3 -> 1 2 3
	// SplitAtIndex, SplitAtIndexX,
	// SplitAtValue, SplitAtValueX,
	// SplitAtFunction, // TODO?
	// Sqrt, // TODO: for ints: ceil/floor?
	Subtract, SubtractX,
	Sum, SumX, // 1,2,3 -> 6
	Sums, SumsX, // 1,2,3 -> 1,3,6
	Swap,
	SwapUnder, // swap two elements under top
	SwapWithIndex,
	// SwapN - swap with top with nth / n from top
	// SwapNM
	Tail, TailX, // everything but first
	...
	fn from(token_str: &str) -> Self {
		...
		if let Some(_token_str) = token_str.strip_prefix("'") {
			...
			match token_str {
				"abs" => Abs, "abs!" => AbsX,
				"add" => Add, "add!" => AddX,
				"alleq" => IsAllEqual, "alleq!" => IsAllEqualX,
				// "allne" => IsAllNotEqual, "allne!" => IsAllNotEqualX,
				"at" => AtIndex, "at!" => AtIndexX,
				"bottom" => Bottom,
				"dec" => Decrease, "dec!" => DecreaseX,
				"diff" => Differences, "diff!" => DifferencesX,
				"digits" => Digits, "digits!" => DigitsX,
				"divint" => DivideInt, "divint!" => DivideIntX,
				"dup" => DuplicateTop,
				...
				"eq" => IsEqual, "eq!" => IsEqualX,
				"first" => First, "first!" => FirstX,
				"head" => Head, "head!" => HeadX,
				"imaxf" => IndexOfMaxFirst, "imaxf!" => IndexOfMaxFirstX,
				"imaxl" => IndexOfMaxLast, "imaxl!" => IndexOfMaxLastX,
				"iminf" => IndexOfMinFirst, "iminf!" => IndexOfMinFirstX,
				"iminl" => IndexOfMinLast, "iminl!" => IndexOfMinLastX,
				"inc" => Increase, "inc!" => IncreaseX,
				"join" => Join, "join!" => JoinX,
				"joindigits" => JoinDigits, "joindigits!" => JoinDigitsX,
				"last" => Last, "last!" => LastX,
				// "map" => Map,
				"max" => Max, "max!" => MaxX,
				"min" => Min, "min!" => MinX,
				"mod" => ModuloRemEuclid, "mod!" => ModuloRemEuclidX,
				"modf" => ModuloFake, "modf!" => ModuloFakeX,
				// TODO: unify/rename with dup! ? (e2dc6d)
				//"move" => Move, // noop
				"movefrom" => MoveFromIndex,
				"moveto" => MoveToIndex,
				"movetobottom" => MoveToBottom,
				"mul" => Multiply, "mul!" => MultiplyX,
				"ne" => IsNotEqual, "ne!" => IsNotEqualX,
				"neg" => Negate, "neg!" => NegateX,
				"not" => Not, "not!" => NotX,
				"pop" => Pop,
				"prod" => Product, "prod!" => ProductX,
				"prods" => Products, "prods!" => ProductsX,
				"range0excl" => Range0Excluding, "range0excl!" => Range0ExcludingX,
				"range0incl" => Range0Including, "range0incl!" => Range0IncludingX,
				"range1excl" => Range1Excluding, "range1excl!" => Range1ExcludingX,
				"range1incl" => Range1Including, "range1incl!" => Range1IncludingX,
				"rev" => Reverse, "rev!" => ReverseX,
				"revstack" => ReverseStack, "revstack!" => ReverseStackX,
				"sliceexclexcl" => SliceExcludingExcluding, "sliceexclexcl!" => SliceExcludingExcludingX,
				"sliceexclincl" => SliceExcludingIncluding, "sliceexclincl!" => SliceExcludingIncludingX,
				"sliceexclfrom" => SliceExcludingFrom, "sliceexclfrom!" => SliceExcludingFromX,
				"sliceexclto" => SliceExcludingTo, "sliceexclto!" => SliceExcludingToX,
				"sliceinclexcl" => SliceIncludingExcluding, "sliceinclexcl!" => SliceIncludingExcludingX,
				"sliceinclincl" => SliceIncludingIncluding, "sliceinclincl!" => SliceIncludingIncludingX,
				"sliceinclfrom" => SliceIncludingFrom, "sliceinclfrom!" => SliceIncludingFromX,
				"sliceinclto" => SliceIncludingTo, "sliceinclto!" => SliceIncludingToX,
				"sort" => Sort, "sort!" => SortX,
				"split" => Split, "split!" => SplitX,
				// "spliti" => SplitAtIndex, "spliti!" => SplitAtIndexX,
				// "splitv" => SplitAtValue, "splitv!" => SplitAtValueX,
				"sub" => Subtract, "sub!" => SubtractX,
				"sum" => Sum, "sum!" => SumX,
				"sums" => Sums, "sums!" => SumsX,
				"swap" => Swap,
				"swapunder" => SwapUnder,
				"swapwith" => SwapWithIndex,
				"tail" => Tail, "tail!" => TailX,
				"top" => Top,
